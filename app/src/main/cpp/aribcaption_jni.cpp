...
Java_com_daig0rian_mirakurun_tvinput_AribCaptionFilter_nativeCreate(
        JNIEnv*, jclass, jint frameWidth, jint frameHeight)
{
    ...
    if (!session->decoder.Initialize(EncodingScheme::kAuto, CaptionType::kCaption)) {
        LOGE("Decoder::Initialize failed");
        delete session;
        return 0;
    }

    if (!session->renderer.Initialize(CaptionType::kCaption,
                                      FontProviderType::kAuto,
                                      TextRendererType::kAuto)) {
        LOGE("Renderer::Initialize failed");
        delete session;
        return 0;
    }

    session->renderer.SetFrameSize(frameWidth, frameHeight);
    LOGI("created session frame=%dx%d", frameWidth, frameHeight);
    return reinterpret_cast<jlong>(session);
}

JNIEXPORT void JNICALL
Java_com_daig0rian_mirakurun_tvinput_AribCaptionFilter_nativeSetFrameSize(
...
// Returns true if a caption was decoded and appended to the renderer.
// pts is in milliseconds (matching libaribcaption's Decode/Render API).
// pesPayload is the PES private-data payload (starts with data_identifier=0x80).
JNIEXPORT jboolean JNICALL
Java_com_daig0rian_mirakurun_tvinput_AribCaptionFilter_nativeDecode(
...
// Returns an array of CaptionImage objects.
// Each image carries its own durationMs (same value for all images in one render call).
JNIEXPORT jobjectArray JNICALL
Java_com_daig0rian_mirakurun_tvinput_AribCaptionFilter_nativeRender(
        JNIEnv* env, jclass, jlong handle, jlong ptsMs)
{
    jclass captionImageClass = env->FindClass(
            "com/daig0rian/mirakurun/tvinput/CaptionImage");
    ...
    for (size_t i = 0; i < images.size(); i++) {
        ...
        if (AndroidBitmap_lockPixels(env, bitmap, &pixels) == ANDROID_BITMAP_RESULT_SUCCESS) {
            ...
            // libaribcaption: RGBA8888 with img.stride bytes per row
            // Android ARGB_8888: also RGBA in memory; stride may differ → copy row by row
...
JNIEXPORT void JNICALL
Java_com_daig0rian_mirakurun_tvinput_AribCaptionFilter_nativeFlush(
...
JNIEXPORT void JNICALL
Java_com_daig0rian_mirakurun_tvinput_AribCaptionFilter_nativeDestroy(
        JNIEnv*, jclass, jlong handle)
{
    if (handle == 0) return;
